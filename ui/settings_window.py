...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def create_widgets(self):
        ...
        self.video_folder=ctk.CTkEntry(
            frame,
            height=40,
            corner_radius=12
        )

        self.video_folder.pack(
            fill="x",
            padx=25,
            pady=(8,20)
        )

        self.video_folder.insert(
            0,
            self.settings.get("video_folder","")
        )
        ...
        self.quality.set(
            str(self.settings.get("quality","Максимальное"))
        )

        ctk.CTkLabel(
            frame,
            text="Максимум сезонов",
            font=("Segoe UI",15)
        ).pack(anchor="w",padx=25)

        self.max_seasons=ctk.CTkEntry(
            frame,
            height=40,
            corner_radius=12
        )

        self.max_seasons.pack(
            fill="x",
            padx=25,
            pady=(8,20)
        )

        self.max_seasons.insert(
            0,
            str(self.settings.get("max_seasons",5))
        )
        ...
        self.max_episodes=ctk.CTkEntry(
            frame,
            height=40,
            corner_radius=12
        )

        self.max_episodes.pack(
            fill="x",
            padx=25,
            pady=(8,20)
        )

        self.max_episodes.insert(
            0,
            str(self.settings.get("max_episodes",10))
        )
