...
class Downloader:
    ...
    def get_format(self, quality):
        ...
        return f"bestvideo[height<={quality}]" "+bestaudio/" f"best[height<={quality}]"
    ...
    def download_episode(self, item, settings, show_name, callback):
        ...
        if season and episode:
            ...
            folder = os.path.join(settings["save_path"], show_name)
        ...
        if estimated_size:
            ...
            callback("log", f"Расчётный размер: {size_mb:.2f} MB")
            callback("log", f"Свободное место: {free_mb:.2f} MB")
    ...
    def download_all(self, selected, settings, show_name, callback):
        ...
        for index, item in enumerate(selected, 1):
            ...
            result = self.download_episode(item, settings, show_name, callback)
